...
-- 15. Durcissement RBAC: UPDATE profiles sensible via RPC uniquement
CREATE OR REPLACE FUNCTION app_private.is_admin()
RETURNS BOOLEAN
LANGUAGE SQL
SECURITY DEFINER
...
-- 16. Modèle CEO-centric: Fenelon seul administrateur, permissions JSONB seules
CREATE OR REPLACE FUNCTION app_private.is_admin()
RETURNS BOOLEAN
LANGUAGE SQL
SECURITY DEFINER
...
BEGIN
  IF lower(NEW.email) = 'zamwanapatricia@gmail.com' THEN
    UPDATE profiles
    SET permissions = '{"leads": true, "studio": true, "coordination": true, "projects": true, "tasks": true, "knowledge": true, "calendar": true, "treasury": true}'::jsonb
    WHERE id = NEW.id;
  ELSIF lower(NEW.email) = 'zainagodlive28@gmail.com' THEN
    UPDATE profiles
    SET permissions = '{"leads": true, "studio": true, "coordination": true, "projects": true, "tasks": true, "knowledge": true, "calendar": true, "brand": true}'::jsonb
    WHERE id = NEW.id;
  ELSIF lower(NEW.email) = 'princebagheni@gmail.com' THEN
    UPDATE profiles
    SET permissions = '{"coordination": true, "projects": true, "tasks": true}'::jsonb
    WHERE id = NEW.id;
  END IF;
